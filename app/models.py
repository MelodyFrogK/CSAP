...
class ControlItem(BaseModel):
    ...
    explanation: str = Field("", description="점검항목 해설")
...
class ItemResult(BaseModel):
    ...
    sheet: str
    row: int
    operation: str = Field("", description="운영여부: 운영 / 미운영 / 해당없음 / 확인불가")
    status: str = Field("", description="운영 현황(이행 내용)")
    related_docs: str = Field("", description="관련문서(정책/지침 등)")
    evidence: str = Field("", description="운영 증적(자산/파일 등)")
    improvement: str = Field("", description="보완사항(추가 작성 열)")
